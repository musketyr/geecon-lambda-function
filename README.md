...
```bash
aws lambda create-function --function-name geecon-lambda-function \
...
```bash
aws lambda invoke --function-name geecon-lambda-function --payload '{"resource": "/{proxy+}", "path": "/ping", "httpMethod": "GET"}' build/response.txt
